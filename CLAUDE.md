...
FFT-TFA is a research-notes repository developing FFT-accelerated Transformation Field Analysis (TFA) for elastoplastic composite homogenization. `code/` holds a single Python/numpy file (`MAIN.py`) that is being built up step by step. The offline stage is implemented from the mesh through the global stiffness `get_K`; everything after it (`get_F_macrostrain`, `get_F_eigenstrain`, `solve_for_displacements`, `get_partition_average_strain`, the returns of `solve_influence_function`, and the whole online stage) is still a `pass` stub or a short wish-list of calls to stubs. There is no dependency manifest, linter, or test suite. Do not assume a build/lint/test toolchain beyond "Python with numpy" (the local environment has Python 3.12 and numpy 2.x; scipy is not installed); check with the user before adding commands or dependencies here. Functions are checked with throwaway scripts in the scratchpad (patch tests, symmetry, null-space counts), not with tests added to the repo.
...
- **Geometry**: 3D, 9 elements and 3 partitions per side by default, with the inclusion equal to the centre partition (`inclusion_side_length` = domain / 3, volume fraction 1/27). Each partition must hold a single material, so inclusion edges must fall on partition edges (and element edges); the config block enforces this with two checks (elements per side divisible by partitions per side, and the centred inclusion having a whole number ≥ 1 of matrix partitions on each side). Element materials come from element-centre geometry, and all indexing code is written in terms of the config counts rather than literal 9 or 3. Only the matrix yields; the inclusion stays elastic.
...
- **`K`** is dense, (3·n³)² with n elements per side (periodic: n³ unique nodes), assembled from `get_K_element` (weight · Σ_g Bᵀ L B). It has exactly 3 zero modes (rigid translations; the periodic wrap removes rotations), so the displacement solve must pin one node; a dense `np.linalg.solve` is the intended approach at this mesh size.
