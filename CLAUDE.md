...
`get_B` returns one array of shape (8, 6, 24): B at the 2×2×2 Gauss points (natural coordinates ±1/√3, weights 1, jacobian h³/8), identical for every element because all elements are the same cube. Displacement DOFs are component-fastest: DOF = component + 3·node, so an element's 24 columns are [u1,u2,u3 of corner 0, then corner 1, …]. The 6 rows follow the ER-2 order (11,22,33,12,23,13) with engineering shear.
